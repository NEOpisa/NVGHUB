...
function extrude(shape: THREE.Shape, depth: number, holes?: THREE.Path[]) {
  ...
  const g = new THREE.ExtrudeGeometry(shape, {
    ...
    bevelEnabled: true, bevelThickness: 0.045, bevelSize: 0.04, bevelSegments: 3,
  });
  g.center();
  return g;
}
...
export function buildServiceIcon(kind: ServiceKind, metal: THREE.Material, accent: THREE.Material): THREE.Group {
  ...
  if (kind === "site") {
    ...
    add(new THREE.TorusGeometry(0.85, 0.16, 28, 72), metalParts, [0, 0.2, 0]);
    add(new THREE.CylinderGeometry(0.1, 0.1, 1.0, 24), metalParts, [0.75, -0.65, 0], [0, 0, Math.PI / 4]);
    ...
    add(new THREE.CylinderGeometry(0.55, 0.55, 1.5, 40), metalParts, [0, 0.05, 0]); // corpo
    add(new THREE.ConeGeometry(0.55, 0.7, 40), accentParts, [0, 1.15, 0]); // bico
    // vigia (anel + lente), espelhada frente/trás
    addBoth(new THREE.TorusGeometry(0.2, 0.05, 20, 48), metalParts, 0.55, [0, 0.35]);
