...
// Loop.
const LOOP_DUR = 1.3 * 1.3 * 1.3 * 1.3;

// Controla se a animação de entrada já foi executada neste ciclo de página.
...
export default function LogoItem({
  ...
  useEffect(() => {
    // Ao voltar para o hero, evita fade/"pulo" de reaparição.
  ...
  const mats = useMemo(() => {
    ...
    const purple = new THREE.MeshPhysicalMaterial({
      ...
      metalness: 0.92,
      roughness: 0.14,
      emissive: new THREE.Color("#b14bff"),
  ...
  const { entranceGroup, loopGroup, entranceMeshes, loopMeshes, disposables } =
    useMemo(() => {
      ...
      const ent = buildLogoChunks(1, 1); // cacos
      const lp = buildLogoChunks(1, 1); //  cacos
      const entranceMeshes = [
        ...make(ent.white, mats.white, true),
        ...make(ent.purple, mats.purple, false),
      ];
      const loopMeshes = [
        ...make(lp.white, mats.white, false),
  ...
  // Mantém o tracking da âncora mesmo invisível e sem interpolação
  // para evitar "pulo" ao voltar ao hero.
  useDomAnchor(groupRef, anchorRef, { z: 0, lerp: 1, enabled: true });
  ...
  useLayoutEffect(() => {
    ...
    if (entrancePlayed) {
      ...
      entranceGroup.rotation.y = -Math.PI * 1.6;
      ...
      entranceMeshes.forEach((m) => {
        ...
        gsap.set(m.position, {
          x:
            pivot.x +
            dir.x * (5 + Math.random() * 3) +
            (Math.random() - 0.5) * 3,
          y:
            pivot.y +
            dir.y * (5 + Math.random() * 3) +
            (Math.random() - 0.5) * 3,
      ...
      tl.to(glow, { current: 1.4, duration: 0.22, ease: "power2.out" }, 1.5).to(
        glow,
        {
          current: 0,
          duration: 0.7,
  ...
  useFrame((state) => {
    ...
    const hiddenByContext = suppressed.current || !enabledRef.current;
    const target = hiddenByContext ? 0 : 1;
    ...
    if (root) {
      // Se o hero não está visível, some imediatamente (sem "vazar" para outras áreas).
      root.visible = !hiddenByContext && f > 0.01;
